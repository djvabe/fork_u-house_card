/**
 * Fork_U-House_Card v12.0 (AI Storyteller Edition) - FIXED VERSION V3
 ...
 * * FIX: Responsive canvas and card height for full-screen view.
 * * RESTORED: Original image selection logic (seasonal/weather/day-night).
 */
...
const TRANSLATIONS = {
    ...
    hu: {
        ...
        advice_snow_soon: "<span class='value-pill pill-1'>❄️ <b>TÉLI FIGYELMEZTETÉS</b></span>  <span class='value-pill'><b>{time}</b></span> körül havazás várható. Az útviszonyok gyorsan romolhatnak, vezess óvatosan!",
...
class ForkUHouseCard extends HTMLElement {
    ...
    static getStubConfig() {
      return {
        ...
        rooms: [{ name: "Hőmérséklet", entity: "sensor.otthon_homerseklet", x: 10, y: 10, weight: 0 }]
      };
    }
    ...
    _generateAIStatus(median) {
        ...
        if (['lightning', 'lightning-rainy', 'hail'].includes(condition)) { msg = this._t('alert_storm'); level = "danger"; }
            ...
            else if (['rainy', 'pouring'].includes(condition)) { msg = this._t('advice_rain_now', {val: wObj.attributes.precipitation || "~"}); level = "warn"; }
            else if (['snowy', 'snowy-rainy'].includes(condition)) { msg = this._t('advice_snow_now'); level = "warn"; }
            else if (uvVal !== null && uvVal > 6) { msg = this._t('alert_uv_high', {val: uvVal}); level = "warn"; }
    ...
    _render() {
      this.shadowRoot.innerHTML = `
        <style>
          ...
          .card {
              position: relative; display: flex; flex-direction: column; width: 100%; height: 100%; min-height: 350px;
          ...
          .gradient-layer { background: linear-gradient(to bottom, rgba(0,0,0,0.2) 0%, transparent 40px); position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: 0; }
          ...
          .badge { position: absolute; transform: translate(-50%, -50%); padding: 6px 12px; border-radius: 16px; background: rgba(20, 20, 25, 0.75); backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.15); box-shadow: 0 4px 8px rgba(0,0,0,0.4); display: flex; align-items: center; gap: 8px; pointer-events: auto; }
          ...
          .footer { position: absolute; bottom: 0; left: 0; width: 100%; z-index: 5; background: rgba(10, 10, 15, 0.25); backdrop-filter: blur(15px); border-top: 1px solid rgba(255,255,255,0.05); padding: 12px 16px; display: flex; align-items: center; gap: 12px; box-sizing: border-box; transition: background 0.3s; min-height: 60px; }
          ...
          .value-pill { background: rgba(20, 20, 25, 0.75); backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.15); box-shadow: 0 4px 8px rgba(0,0,0,0.4); padding: 2px 8px; border-radius: 20px; color: rgba(255, 255, 255, 0.6); white-space: nowrap; }
    ...
    _animate() {
      ...
      let wState = wEnt ? this._hass.states[wEnt]?.state : "";
      ...
      if (['rainy','pouring','lightning','lightning-rainy'].includes(wState)) { this._drawRain(wState === 'pouring' ? 2 : 1, windDirX, moveSpeed); }
      else if (['snowy','snowy-rainy'].includes(wState)) { this._drawSnow(windDirX, moveSpeed); } 
      if (['lightning','lightning-rainy'].includes(wState)) this._handleLightning();
      if (this._flashOpacity > 0) { this._ctx.fillStyle = `rgba(255, 255, 255, ${this._flashOpacity})`; this._flashOpacity -= 0.05; }
      this._animationFrame = requestAnimationFrame(() => this._animate());
    }

    _drawStars(coverage) {
        const visibility = Math.max(0, 1 - (coverage / 80)); if (visibility <= 0) return;
    ...
    _drawFog(speed) {
        if (this._fogParticles.length < 10) {
            this._fogParticles.push({ x: Math.random() * this._canvas.width, y: this._canvas.height - (Math.random() * 50), radius: 50 + Math.random() * 50, speed: (Math.random() * 0.2) + 0.05 });
        }
    ...
    _drawRain(intensity, windDirX, windSpeed) {
      ...
      for (let i = 0; i < this._particles.length; i++) {
          const p = this._particles[i]; this._ctx.moveTo(p.x, p.y); this._ctx.lineTo(p.x + angleX, p.y + p.length);
    ...
    _drawSnow(windDirX, windSpeed) {
      ...
      for (let i = 0; i < this._particles.length; i++) {
          const p = this._particles[i]; this._ctx.moveTo(p.x, p.y); this._ctx.arc(p.x, p.y, p.radius, 0, Math.PI * 2);
